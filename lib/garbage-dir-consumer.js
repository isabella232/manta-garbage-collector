...
function GarbageDirConsumer(opts) {
    ...
    // We're going to use self.readBatchSize later to build a query, and
    // self.readBatchDelay for setting a timeout, so we want to be 100% sure
    // they're valid numbers.
    assert.number(self.readBatchDelay, 'readBatchDelay');
    assert.number(self.readBatchSize, 'readBatchSize');
